...
CHAMPION *recursive_search(CHAMPION **list, char *input);
...
CHAMPION *recursive_search(CHAMPION **list, char *input) {
	...
	if (data != NULL) {
		if (!(strcmp(data->name, input))) {
			...
			data = data->link;
			return recursive_search(&data, input);
		}
	}
	else {
		data = NULL;
...
int recursive_insert(CHAMPION **list, char *input) {
	CHAMPION *data = *list;
	if (data != NULL) {
		if (!(strcmp(data->name, input))) {
			return 0;
			...
			data = data->link;
			return recursive_insert(&data, input);
		}
	}
	else {
		return 1;
...
void Search(CHAMPION **next, CHAMPION **list) {					// 1. Search
	...
	data = recursive_search(&data, input);
	if (data != NULL) {
...
void Insert(CHAMPION **list, CHAMPION **prev) {					// 2. Insert
	...
	if (recursive_insert(list, data->name)) {
		printf("hp : ");
		scanf("%d", &data->hp);
		printf("mp : ");
		scanf("%d", &data->mp);
